...
(function () {
  ...
  function extractMetadata(context, qualities, selected) {
    ...
    return {
      ...
      duration: videoObject.duration || textNearLabel(doc, /duration/i),
      uploadDate: videoObject.uploadDate || textNearLabel(doc, /added|date/i),
      views: extractInteractionCount(videoObject, 'WatchAction') || textNearLabel(doc, /views/i),
      likes: extractInteractionCount(videoObject, 'LikeAction') || '',
      rating: textNearLabel(doc, /rating/i),
  ...
  function extractUploader(doc) {
    const label = Array.from(doc.querySelectorAll('.label, dt, .info .title')).find((el) => /uploaded by|uploader/i.test(el.textContent || ''));
    if (label) {
  ...
  function findNextPageLink(doc) {
    ...
    return links.find((link) => /^(next|>|next\s*>|older)$/i.test(normalizeWhitespace(link.textContent))) || null;
  }
  ...
  function findFirstPageLink(doc) {
    ...
    return links.find((link) => /^(first|<<|<\s*<)$/i.test(normalizeWhitespace(link.textContent))) || null;
  }
  ...
  function totalPageNumber(doc) {
    ...
    const text = normalizeWhitespace(Array.from(doc.querySelectorAll('[id*="pagination"], .pagination, .page, .paging'))
      ...
      .join(' '));
    const totalMatch = text.match(/(?:of|\/|total)\s*(\d{1,6})(?:\s*pages?)?/i)
      || text.match(/(?:page)\s*\d{1,6}\s*(?:of|\/)\s*(\d{1,6})/i);
